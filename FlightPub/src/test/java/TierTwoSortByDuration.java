...
public class TierTwoSortByDuration {
    ...
    public void sortByDuration() {
        ...
        //System.out.println("None Important List: ");
        for(int i=0;i<noneImportantList.size();i++) {
            //System.out.println(noneImportantList.get(i).getLegs().get(0).getDuration());

            long duration = noneImportantList.get(i).getLegs().get(0).getDuration();

            if(i>0) {
                previousDuration = noneImportantList.get(i - 1).getLegs().get(0).getDuration();
            }
        ...
        //set duration slider to very important, facilitating sort by duration
        SearchCriteria durationImportant = new SearchCriteria("1","5","1","1");

        LinkedList<FlightOptionContainer> durationImportantList =FlightOptionContainer.rankAndSortFlights(madridToMiamiFlights,durationImportant);
        ...
        for(int i=0;i<durationImportantList.size();i++) {
            System.out.println(durationImportantList.get(i).getLegs().get(0).getDuration());
            long duration = durationImportantList.get(i).getLegs().get(0).getDuration();

            if(i>0) {
                previousDuration = durationImportantList.get(i - 1).getLegs().get(0).getDuration();
            }
