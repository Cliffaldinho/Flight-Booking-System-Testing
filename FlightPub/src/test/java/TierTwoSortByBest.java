...
public class TierTwoSortByBest {
    ...
    public void sortByBest() {
        ...
        for(int i=0;i<noneImportantList.size();i++) {
            ...
            double rating = noneImportantList.get(i).getLegs().get(0).getAirlineRating();

            if(i>0) {
                previousRating = noneImportantList.get(i - 1).getLegs().get(0).getAirlineRating();
            }
        ...
        //set comfort slider to very important, facilitating sort by best
        SearchCriteria ratingImportant = new SearchCriteria("1","1","5","1");

        LinkedList<FlightOptionContainer> ratingImportantList =FlightOptionContainer.rankAndSortFlights(madridToMiamiFlights,ratingImportant);
        ...
        for(int i=0;i<ratingImportantList.size();i++) {
            System.out.println(ratingImportantList.get(i).getLegs().get(0).getAirlineRating());
            double rating = ratingImportantList.get(i).getLegs().get(0).getAirlineRating();

            if(i>0) {
                previousRating = ratingImportantList.get(i - 1).getLegs().get(0).getAirlineRating();
            }
